...
import { IoSaveOutline, IoCloseOutline } from "react-icons/io";
...
export default function UpdateStateForm({
  ...
  const handleSubmit = async (e) => {
    ...
    const isValid = requiredFields.every(
      (field) =>
        newElementName !== null &&
        newElementName.trim() !== "" &&
        newElementAcronym !== null &&
        newElementAcronym.trim() !== ""
    );
    if (isValid) {
      let element = await getElementById(selectedOption);
      element.name = newElementName;
      element.acronym = newElementAcronym;
  ...
  const handleSetSelectedOption = (option) => {
    ...
    setNewElementName(element.name);
    setNewElementAcronym(element.acronym);
  };
